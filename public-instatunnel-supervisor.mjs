...
const outFile = path.join(__dirname, "public-insta-supervisor.out.log");
const errFile = path.join(__dirname, "public-insta-supervisor.err.log");
const urlFile = path.join(__dirname, "public-url.txt");
const pidFile = path.join(__dirname, "public-insta-supervisor.pid");
...
async function isHealthyPage(url) {
  try {
...
async function isPublicHealthy(url) {
  return await isHealthyPage(url);
}

function extractTunnelUrl(text) {
  const match = text.match(/https:\/\/[a-z0-9.-]+\.instatunnel\.my/i);
  return match?.[0] ?? null;
}
...
async function startTunnel() {
  const child = spawn(
    ...
    [
      ...
      "/s",
      "/c",
      `"npx instatunnel ${TARGET_PORT} --subdomain ${SUBDOMAIN} --request-timeout ${REQUEST_TIMEOUT}"`,
    ],
    {
      cwd: __dirname,
  ...
  child.on("exit", async (code, signal) => {
    wrapped.closed = true;
    await logLine(errFile, `instatunnel exit code=${code ?? "null"} signal=${signal ?? "null"}`);
  });
...
async function waitForTunnelUrl(wrapped) {
  ...
  while (!shuttingDown && !wrapped.closed && Date.now() - startedAt < 25_000) {
    if (wrapped.url) {
      return wrapped;
    }
    await sleep(250);
  ...
  throw new Error("No se pudo obtener URL publica de InstaTunnel");
}
